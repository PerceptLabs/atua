...
describe('AtuaFS (Browser)', () => {
  ...
  it('should write and read binary data', async () => {
    const fs = await AtuaFS.create('bin-test-' + Date.now());
  ...
  it('should mkdir recursive + readdir', async () => {
    const fs = await AtuaFS.create('dir-test-' + Date.now());
  ...
  it('should stat files and directories', async () => {
    const fs = await AtuaFS.create('stat-test-' + Date.now());
  ...
  it('should readdir with withFileTypes', async () => {
    const fs = await AtuaFS.create('ftype-test-' + Date.now());
    fs.mkdirSync('/typed');
  ...
  it('should rename files', async () => {
    const fs = await AtuaFS.create('rename-test-' + Date.now());
  ...
  it('should unlink files', async () => {
    const fs = await AtuaFS.create('unlink-test-' + Date.now());
  ...
  it('should copy files', async () => {
    const fs = await AtuaFS.create('copy-test-' + Date.now());
  ...
  it('should existsSync correctly', async () => {
    const fs = await AtuaFS.create('exists-test-' + Date.now());
  ...
  it('should persist files in IndexedDB across AtuaFS.create() calls', async () => {
  ...
  it('should handle async operations', async () => {
    const fs = await AtuaFS.create('async-test-' + Date.now());
  ...
  it('should measure OPFS write/read performance', async () => {
    const fs = await AtuaFS.create('perf-test-' + Date.now());
    ...
    console.log(
      `[AtuaFS] Performance — write 1KB: ${avgWrite.toFixed(3)}ms, read 1KB: ${avgRead.toFixed(3)}ms`
    );
