...
describe('AtuaEngine — QuickJS Boot', () => {
  it('should boot QuickJS and report variant', async () => {
    ...
    console.log(`[AtuaEngine] QuickJS boot time: ${bootTime.toFixed(1)}ms`);
  ...
  it('should boot within reasonable time', async () => {
    ...
    console.log(`[AtuaEngine] Boot time: ${bootTime.toFixed(1)}ms`);
...
describe('AtuaEngine — Console Capture', () => {
...
describe('AtuaEngine — require() Built-in Modules', () => {
  ...
  it('require("process") should have env and platform', async () => {
    const engine = await AtuaEngine.create({ env: { TEST_VAR: 'yes' } });
    const result = await engine.eval(`
...
describe('AtuaEngine — require("fs") with AtuaFS', () => {
  ...
  it('should write files and read back', async () => {
    ...
    // Verify write was persisted to AtuaFS
...
describe('AtuaEngine — require() Relative Paths', () => {
  it('should resolve relative require from AtuaFS', async () => {
    const fs = await AtuaFS.create({
...
describe('AtuaEngine — evalFile()', () => {
  it('should eval a file from AtuaFS', async () => {
    const fs = await AtuaFS.create({
...
describe('AtuaEngine — Error Handling', () => {
...
describe('AtuaEngine — Memory Limit', () => {
  it('should reject memory bomb with configurable limit', async () => {
    const engine = await AtuaEngine.create({ memoryLimit: 4 }); // 4MB limit
...
describe('AtuaEngine — Event Emitter', () => {
...
describe('AtuaEngine — Module Caching', () => {
