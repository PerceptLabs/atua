/**
 * AtuaWASI — Browser tests
 *
 * Tests WASI execution in real Chromium with AtuaFS integration.
...
describe('AtuaWASI — WASM Execution (Browser)', () => {
...
describe('AtuaWASI — AtuaFS Integration (Browser)', () => {
  let fs: AtuaFS;

  beforeEach(async () => {
    fs = await AtuaFS.create('wasi-browser-fs');
  });

  it('should access AtuaFS from WASI bindings', async () => {
    fs.writeFileSync('/wasi-test.txt', 'data from atua');
...
describe('BinaryCache — Browser', () => {
  it('should store and retrieve a WASM binary', async () => {
    const fs = await AtuaFS.create('wasi-cache-store-' + Date.now());
  ...
  it('should compute consistent SHA-256 hashes', async () => {
    const fs = await AtuaFS.create('wasi-cache-hash-' + Date.now());
    const cache = new BinaryCache({ fs });
  ...
  it('should persist cache metadata', async () => {
    ...
    const fs = await AtuaFS.create(fsName);
  ...
  it('should clear all cached entries', async () => {
    const fs = await AtuaFS.create('wasi-cache-clear-' + Date.now());
    const cache = new BinaryCache({ fs });
    await cache.init();
...
describe('ProcessManager — WASI Integration (Browser)', () => {
  it('should have execWasm method', async () => {
    const fs = await AtuaFS.create('wasi-pm-browser');
  ...
  it('should execute WASM binary via ProcessManager', async () => {
    const fs = await AtuaFS.create('wasi-pm-exec');
    const binary = buildNoopWasm();
    // Store binary in FS
