/**
 * AtuaDNS — DNS resolution via DNS-over-HTTPS (DoH)
 *
...
export function getDNSModuleSource(): string {
  return `
(function() {
  // AtuaDNS — DNS resolution stubs
  // Real resolution happens via the host AtuaDNS class via fetch.
  ...
  function notImplemented(name) {
    return function() {
      ...
      if (typeof cb === 'function') {
        cb(new Error('dns.' + name + '() requires DoH — use AtuaDNS from the host'));
      } else {
        throw new Error('dns.' + name + '() requires DoH — use AtuaDNS from the host');
      }
    };
  }
  ...
  module.exports.promises = {
    lookup: function() { return Promise.reject(new Error('dns.promises.lookup() requires AtuaDNS')); },
    resolve: function() { return Promise.reject(new Error('dns.promises.resolve() requires AtuaDNS')); },
    resolve4: function() { return Promise.reject(new Error('dns.promises.resolve4() requires AtuaDNS')); },
    resolve6: function() { return Promise.reject(new Error('dns.promises.resolve6() requires AtuaDNS')); },
  };
