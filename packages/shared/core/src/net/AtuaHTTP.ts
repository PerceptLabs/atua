/**
 * AtuaHTTP — Node.js http module backed by MessagePort routing
 *
 * User code calls http.createServer(handler) → Atua intercepts →
...
export interface AtuaIncomingMessage {
  method: string;
  url: string;
...
export interface AtuaServerResponse {
  statusCode: number;
...
export type RequestHandler = (
  req: AtuaIncomingMessage,
  res: AtuaServerResponse,
) => void;
...
/**
 * AtuaHTTPServer — handles incoming requests via MessagePort
 */
export class AtuaHTTPServer {
  ...
  private _createIncomingMessage(req: SerializedHTTPRequest): AtuaIncomingMessage {
    const eventHandlers: Record<string, EventHandler[]> = {};
    let bodyConsumed = false;

    const msg: AtuaIncomingMessage = {
      method: req.method,
      url: req.url,
      headers: req.headers,
      httpVersion: '1.1',
 ...
 */
export function createHTTPServer(handler: RequestHandler): AtuaHTTPServer {
  return new AtuaHTTPServer(handler);
}
...
export function getHTTPModuleSource(): string {
  return `
(function() {
  // AtuaHTTP — Node.js http compatibility layer
  var servers = [];
  var nextServerId = 1;
