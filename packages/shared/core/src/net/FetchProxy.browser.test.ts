 ...
 * Tests end-to-end fetch through NativeEngine + FetchProxy.
...
describe('FetchProxy — QuickJS Fetch Integration', () => {
  ...
  it('should fetch text content via NativeEngine', async () => {
    const fetchProxy = new MockFetchProxy();
...
describe('FetchProxy — evalAsync Expression Capture', () => {
  it('should capture the last expression value', async () => {
    ...
    // Test without any fetch — just eval with module.exports capture
    const result = await engine.eval(`
      var x = 10;
      var y = 20;
      module.exports = x + y;
