/**
 * AtuaTLS — TLS/SSL support via WebCrypto
 *
 ...
 * - tls.connect() → AtuaTCPSocket with TLS flag
 * - tls.createServer() → AtuaHTTPServer with TLS context
 * - Certificate validation stubs
 */

import { AtuaTCPSocket, AtuaTCPServer, type TCPConnectionOptions } from './AtuaTCP.js';
...
export interface TLSSocket extends AtuaTCPSocket {
...
/**
 * Create a TLS connection — wraps AtuaTCPSocket with TLS metadata.
 */
export function tlsConnect(options: TLSConnectionOptions): AtuaTCPSocket & { encrypted: boolean; authorized: boolean; servername: string } {
  const socket = new AtuaTCPSocket({
    ...options,
  ...
  // Add TLS metadata
  const tlsSocket = socket as AtuaTCPSocket & {
    encrypted: boolean;
    authorized: boolean;
    servername: string;
    getPeerCertificate: () => Record<string, unknown>;
