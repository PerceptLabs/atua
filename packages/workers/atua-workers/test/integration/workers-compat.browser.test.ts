 ...
 * in AtuaWorkers. Tests wrangler.toml auto-configuration, bindings,
 * and data persistence across destroy/recreate cycles.
 */
import { describe, it, expect, afterEach } from 'vitest';
import { AtuaWorkers } from '../../src/runtime.js';
import { AtuaKV } from '../../src/bindings/kv.js';
...
describe('Workers Compat — Raw Worker Bundle', () => {
  it('raw Worker bundle processes requests correctly', async () => {
    const kv = new AtuaKV(`raw-compat-kv-${Date.now()}`);
    ...
    expect(await rootRes!.text()).toBe('Raw Worker on Atua');
  ...
  it('raw Worker uses KV + D1 from env', async () => {
    const kv = new AtuaKV(`raw-bindings-kv-${Date.now()}`);
    const db = new AtuaD1(`raw-bindings-db-${Date.now()}`);
...
`;
    ...
    // Use parsed config to create AtuaWorkers (with instance overrides for D1)
    const kv = new AtuaKV(`auto-config-kv-${Date.now()}`);
    const db = new AtuaD1(`auto-config-db-${Date.now()}`);
...
describe('Workers Compat — Persistence', () => {
  it('data persists across AtuaWorkers destroy/recreate (OPFS)', async () => {
    const persistNs = `persist-test-${Date.now()}`;
    const kv = new AtuaKV(persistNs);
    ...
    const kv2 = new AtuaKV(persistNs);
    runtime = await AtuaWorkers.create({
      workers: {
        raw: {
          module: rawWorkerBundle as unknown as WorkerModule,
