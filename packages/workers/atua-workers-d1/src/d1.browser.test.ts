/**
 * AtuaD1 — Browser tests (SQL operations)
 *
 * All tests run in Chromium via Vitest browser mode.
 * Uses real wa-sqlite with IDBBatchAtomicVFS.
 */
import { describe, it, expect, beforeEach, afterEach } from 'vitest';
import { AtuaD1 } from './d1.js';

let d1: AtuaD1;
let dbCounter = 0;

function createD1(): AtuaD1 {
  return new AtuaD1(`test-db-${Date.now()}-${dbCounter++}`);
}
...
describe('AtuaD1 — PreparedStatement', () => {
  beforeEach(async () => {
    d1 = createD1();
...
describe('AtuaD1 — SQL Injection Safety', () => {
  beforeEach(async () => {
    d1 = createD1();
