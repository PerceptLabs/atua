/**
 * AtuaD1 — Cloudflare Workers D1 emulation backed by wa-sqlite + IndexedDB.
 *
...
export class AtuaD1 {
   ...
   * Returns an AtuaD1PreparedStatement that can be bound and executed.
   */
  prepare(sql: string): AtuaD1PreparedStatement {
    return new AtuaD1PreparedStatement(this, sql);
  }
  ...
  async batch<T = Record<string, unknown>>(
    statements: AtuaD1PreparedStatement[],
  ): Promise<D1Result<T>[]> {
    const { sqlite3, db } = await this.ready();
   ...
   * Used by AtuaD1PreparedStatement.
   */
...
export class AtuaD1PreparedStatement {
  private readonly d1: AtuaD1;
  private readonly sql: string;
  private bindings: unknown[] = [];

  constructor(d1: AtuaD1, sql: string) {
    this.d1 = d1;
    this.sql = sql;
  }
   ...
   */
  bind(...values: unknown[]): AtuaD1PreparedStatement {
    const stmt = new AtuaD1PreparedStatement(this.d1, this.sql);
