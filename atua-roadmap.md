# Atua — Implementation Roadmap

> **Companion doc:** `atua-spec.md` (architecture, decisions, cleanroom protocol)  
...
```
This project implements a browser-native runtime from atua-spec.md only.
...
Phase 1:  AtuaFS — Filesystem                   (2-3 days)
Phase 2:  AtuaFS — Multi-Mount + File Watching  (2-3 days)
Phase 3:  Preview Service Worker                    (1-2 days)
Phase 4:  AtuaEngine — QuickJS Integration      (3-5 days)
Phase 5:  AtuaNet — Fetch Proxy                 (2-3 days)
Phase 6:  AtuaProc — Process Management         (2-3 days)
Phase 7:  AtuaPkg — Package Management          (3-5 days)
Phase 8:  AtuaDev — Build Pipeline + HMR        (2-3 days)
Phase 9:  Integration Tests + Example App           (2-3 days)
Phase 10: AtuaWASI — Non-JS Binary Execution    (3-5 days)
Phase 11: AtuaSync — Deno Server Protocol       (3-5 days)
...
catalyst/
├── packages/
│   ├── core/                    <- @aspect/atua-core
│   │   ├── src/
│   │   │   ├── fs/              <- AtuaFS (Phase 1)
│   │   │   ├── engine/          <- AtuaEngine (Phase 4)
│   │   │   ├── net/             <- AtuaNet (Phase 5)
│   │   │   ├── proc/            <- AtuaProc (Phase 6)
│   │   │   └── index.ts
│   │   ├── package.json
│   │   └── tsconfig.json
│   ├── pkg/                     <- @aspect/atua-pkg (Phase 7)
│   └── dev/                     <- @aspect/atua-dev (Phase 8)
├── examples/
...
1. `packages/core/src/fs/AtuaFS.ts`
   - Static factory: `AtuaFS.create(name: string): Promise<AtuaFS>`
...
2. `packages/core/src/fs/types.ts` — AtuaFSConfig, MountConfig, WatchCallback, FileEntry

3. `packages/core/src/fs/index.ts` — barrel exports

4. `packages/core/src/fs/AtuaFS.test.ts` (Node — pure logic via ZenFS InMemory backend)
...
5. `packages/core/src/fs/AtuaFS.browser.test.ts` (Browser — OPFS via Playwright)
   - Same test cases as above but running against OPFS backend in real Chromium
   - OPFS persistence: write file, destroy AtuaFS instance, recreate, verify file exists
   - OPFS performance: time 100 sequential writes, verify <1ms average per 1KB write
   - Concurrent access: two AtuaFS instances reading/writing same OPFS store
...
- [ ] Files persist across AtuaFS.create() calls in OPFS
- [ ] Error handling: meaningful errors for missing files
- [ ] rawFs works with isomorphic-git (`git.init({ fs: atuaFs.rawFs, dir: '/test' })`)
...
1. Update `AtuaFS.create()` to accept mount configuration:
...
**Goal:** A Service Worker that serves files from AtuaFS as HTTP responses.
...
1. `packages/core/src/net/PreviewSW.ts` — SW source
   ...
   - Intercepts fetch: static files from AtuaFS, SPA fallback, /api/* passthrough
...
- [ ] `pnpm test:browser` passes:
  ...
  - [ ] Write file to AtuaFS -> fetch via SW -> correct content returned
  ...
  - [ ] SW receives MessagePort and gets full AtuaFS access via Port backend
...
**Goal:** QuickJS-WASM running user code with host bindings to AtuaFS. The core runtime.

**What gets built:**

1. `packages/core/src/engine/AtuaEngine.ts`
   - JSPI/Asyncify variant detection + dynamic import
   - Factory: `AtuaEngine.create(config)`
...
3. `packages/core/src/engine/require.ts` — CJS module loader:
   ...
   2. Relative paths -> resolve against AtuaFS, eval
   3. /node_modules/{name} -> read from AtuaFS
...
- require() reads source from AtuaFS, evals with module/exports/__filename/__dirname wrappers
...
- [ ] `pnpm test:browser` passes — all QuickJS-WASM tests run in real Chromium:
  ...
  - [ ] require('fs').readFileSync reads from AtuaFS (OPFS backend)
  - [ ] require('fs').writeFileSync writes to AtuaFS, persists
  ...
  - [ ] require('./relative') resolves from AtuaFS
...
3. Wire FetchProxy into AtuaEngine
...
2. AtuaProcess.ts — pid, stdin, stdout, stderr, exitCode, kill, events
3. worker-template.ts — Worker entry: boots QuickJS + AtuaFS, executes command, streams stdio
...
- Worker gets AtuaFS access via Port backend over MessagePort
...
- [ ] `pnpm test:browser` passes:
  ...
  - [ ] Child process has AtuaFS access via Port backend (read/write files)
...
5. `packages/pkg/src/Lockfile.ts`
   - Read/write `atua-lock.json` in project root
...
- Registry fetches go through AtuaNet's fetch proxy (domain allowlist should include registry.npmjs.org and esm.sh by default)
...
- [ ] `pnpm test:browser` passes:
  ...
  - [ ] Lockfile written to AtuaFS with versions + SHA-256 integrity hashes
  - [ ] Second install is instant (OPFS cache hit, no network traffic — verify via fetch spy)
  - [ ] installAll() reads package.json from AtuaFS, installs all listed deps
...
2. `packages/dev/src/HMRManager.ts` — AtuaFS.watch() -> build -> emit update event
...
- Read source from AtuaFS, write output to AtuaFS /dist/
...
- [ ] `pnpm test:browser` passes:
  - [ ] Write TSX source to AtuaFS → build → /dist/app.js exists with valid JS
  - [ ] Content-hash cache: identical source → second build returns instantly (0ms, no esbuild invocation)
  - [ ] Modify source file → AtuaFS.watch() fires → rebuild triggers → HMR event emitted
...
2. `packages/core/src/catalyst.ts` — top-level Atua.create() factory
...
4. **Integration test suite** (`packages/core/src/integration.browser.test.ts`):
   - Full round-trip: create Atua → write React app source → build with esbuild → serve via SW → fetch rendered page → verify HTML content
   ...
   - Persistence: write files → destroy Atua → recreate with same name → verify files exist in OPFS
...
5. **Node.js compatibility matrix** (`packages/core/src/compat/node-compat.browser.test.ts`):
   ...
   A test file that runs ~200 Node.js API calls through AtuaEngine in real Chromium. Each test:
...
7. `examples/basic/` — minimal HTML page:
   - Imports @aspect/atua-core
   - Creates a Atua instance
   - Writes a React "Hello from Atua" app to virtual filesystem
   - Builds with AtuaDev
...
8. Performance benchmarks (`packages/core/src/compat/perf.browser.test.ts`):
   - AtuaFS write 1KB: target <1ms (run 100x, report average + p95)
   - AtuaFS read 1KB: target <0.5ms
   - AtuaFS write 100KB: target <5ms
   - AtuaFS readdir 100 files: target <10ms
...
9. Bundle size audit:
   - @aspect/atua-core: target ~600KB (log actual)
   - @aspect/atua-pkg: target ~50KB
   - @aspect/atua-dev: target ~30KB
...
- [ ] Example app: opens in browser, renders "Hello from Atua" in preview iframe
...
**Goal:** Run programs compiled to wasm32-wasi inside Atua. Python scripts, Rust CLIs, Go tools — anything that compiles to WASI runs in the browser.
...
1. `packages/wasi/src/AtuaWASI.ts`
   - Wraps Wasmer-JS (`@aspect/atua-wasi` package, separate from core)
   - `AtuaWASI.create(config: WASIConfig): Promise<AtuaWASI>`
   - `exec(wasmBinary: Uint8Array, args: string[], env?: Record<string,string>): Promise<ExecResult>`
   - `execFile(path: string, args: string[]): Promise<ExecResult>` — read .wasm from AtuaFS, then exec
   - WASI filesystem mapping: WASI fd_read/fd_write → AtuaFS
   - WASI stdout/stderr capture → same pattern as AtuaProc

2. `packages/wasi/src/WASIBindings.ts`
   - Map WASI system calls to Atua layers:
     - `fd_read`, `fd_write`, `path_open`, `path_filestat_get` → AtuaFS
...
4. Wire into AtuaProc:
   - ProcessManager detects `.wasm` files → route to AtuaWASI instead of QuickJS
   - `atua.exec('python script.py')` → boots Python WASI → runs script

5. Tests:
   - Execute a simple WASI binary (compile a hello-world Rust program to wasm32-wasi)
   - WASI reads file from AtuaFS
   - WASI writes file to AtuaFS
...
- AtuaFS mount points map to WASI preopened directories

**Verification:**
- [ ] Hello-world WASI binary runs, stdout captured
- [ ] WASI binary reads from AtuaFS
- [ ] WASI binary writes to AtuaFS
...
**Goal:** Bidirectional filesystem sync between Atua (browser OPFS) and a Deno server (disk). Browser-only works completely. Connect a server to unlock real databases, native modules, production builds. Disconnect — back to browser-only. No mode switch, no data loss.

**Why Deno:**
- Native npm package support via `npm:` specifiers (same packages Atua resolves)
...
1. `packages/sync/src/SyncClient.ts` (browser-side)
   ...
   - Auto-sync mode: push on AtuaFS.watch() events, pull on server notification
...
**Why Hono:**
- 18KB, MIT license
- Official Service Worker adapter (runs in Atua's preview SW)
...
1. `packages/dev/src/HonoIntegration.ts`
   ...
   - Hono routes handle API requests, AtuaFS provides the data layer
...
2. `packages/dev/src/hono-sw-adapter.ts`
   ...
   - Hono gets a `context.env` with AtuaFS access for reading/writing data
...
4. `examples/fullstack/` — example full-stack app:
   ...
   - Backend: Hono routes that read/write todos to AtuaFS as JSON files
...
5. Tests:
   ...
   - Hono route reads from AtuaFS
   - Hono route writes to AtuaFS
...
| 2 | Phase 1 (AtuaFS core) | 4-6 hrs |
| 3 | Phase 2 (Multi-mount + Watcher) | 4-6 hrs |
| 4 | Phase 3 (Preview SW) | 3-4 hrs |
| 5 | Phase 4 (AtuaEngine) | 6-10 hrs |
| 6 | Phase 5 (AtuaNet) | 4-6 hrs |
| 7 | Phase 6 (AtuaProc) | 4-6 hrs |
| 8 | Phase 7 (AtuaPkg) | 6-10 hrs |
| 9 | Phase 8 (AtuaDev) | 4-6 hrs |
